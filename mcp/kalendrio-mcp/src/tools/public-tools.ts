...
export function registerPublicTools(server: McpServer, client: KalendrioClient) {
  server.registerTool(
    ...
    async ({ username, slug }) => {
      try {
        const response = await client.getPublicEventType(username, slug) as {
  ...
  server.registerTool(
    ...
    async ({ username, slug, month, year, timezone }) => {
      try {
        const response = await client.listAvailableDates({ username, slug, month, year, timezone }) as {
          eventType: Record<string, unknown>;
  ...
  server.registerTool(
    ...
    async ({ username, slug, date, timezone }) => {
      try {
        const response = await client.listTimeSlots({ username, slug, date, timezone }) as {
          eventType: Record<string, unknown>;
          host: Record<string, unknown>;
          timeSlots?: Array<{ time: string; endTime: string }>;
        };
        return formatJsonResult({
          eventType: response.eventType,
          host: response.host,
  ...
  server.registerTool(
    ...
    async (input) => {
      try {
        const response = await client.createBooking(input) as {
  ...
  server.registerTool(
    ...
    async ({ username, slug, date, timezone }) => {
      try {
        const eventTypeResponse = await client.getPublicEventType(username, slug) as {
          eventType: Record<string, unknown>;
          host: Record<string, unknown>;
        };
        const slotsResponse = await client.listTimeSlots({ username, slug, date, timezone }) as {
          timeSlots?: Array<{ time: string; endTime: string }>;
        };

        const month = Number(date.slice(5, 7)) - 1;
        const year = Number(date.slice(0, 4));
        const datesResponse = await client.listAvailableDates({ username, slug, month, year, timezone }) as {
          availableDates?: string[];
        };
